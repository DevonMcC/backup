...
NB.* listEOMDates: list end-of-month dates between 2 YYYYmmdd dates;
...
NB.* listEOMDates: list end-of-month dates between 2 YYYYmmdd dates;
NB. left arg is flag: 0: exact EOM dates; 1: only weekday EOM dates.
listEOMDates=: 3 : 0
   0 listEOMDates y                     NB. Does not have to be weekday
:
   'st end'=. y
   assert. (8=#":st) *. 8=#":end        NB. 8-char YYYYmmdd dates
   if. ' '={.0$st do. st=. ".st end.    NB. Ensure numeric
   if. ' '={.0$end do. end=. ".end end.
   assert. (st>0) *. end>0              NB. Positive numbers only
   days=. (toJulian st)+ i. >: -/toJulian end,st
   if. x do. days=. days#~(dow"1 ] 0 100 100#:toGregorian days) e. >:i.5 end.
   eomdts=. (toGregorian days){~I. 2~:/\100|<.100%~toGregorian days
