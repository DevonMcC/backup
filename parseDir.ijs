...
NB.* figureWhichFiles: figure which files we want to select based on one or two sizes.
...
:
   ...
   y=. endSlash y
   if. '/' e. >{.DIRNMS do. y=. y rplc '\/' end.       NB. Ensure correct slash
   areEndSlashes=. +./'/'=~.{:&>DIRNMS
NB.   whamsd=. (top=. DIRNMS i. <(-3~:#y)}.y),I. (DIRNMS{.~&.>#y) e. <y
   whamsd=. (top=. DIRNMS i. <(--.areEndSlashes)}.y),I. (DIRNMS{.~&.>#y) e. <y
...
NB.    end.
   ...
   newnm
NB.EG consolidateBkpsTowardPast 'ciqRB';'C:/Temp/'
)
...
#(<'/Temp/') fileVar_WS_&.>'FLNMS';'FLDTS';'FLSZS';'FLPARENT';'DIRNMS';'DIRDEP' [ closeLog_logger_''
6!:2&.>'''batfl cmds''=. buildBatFl_parseDir_ 10e6;BASEDSK,''/Temp/{machid}{todayDt}/''';'shell batfl'
6!:2&.>'''batfl cmds''=. buildBatFl_parseDir_ 10e6;BASEDSK,''\Users\devon_mccormick\Dropbox\Recent\''';'shell batfl' NB. Work
6!:2&.>'''batfl cmds''=. buildBatFl_parseDir_ 10e6;BASEDSK,''\Users\DevonMcC\Dropbox\Recent\''';'shell batfl' NB. Home - Win7
6!:2&.>'''batfl cmds''=. buildBatFl_parseDir_ 10e6;BASEDSK,''\Documents and Settings\Dev\My Documents\My Dropbox\Recent\''';'shell batfl' NB. Home - XP
6!:2&.>'''batfl cmds''=. buildBatFl_parseDir_ 50e6;''J:\''';'shell batfl' NB. Network
6!:2&.>'''batfl cmds''=. buildBatFl_parseDir_ 20e6;''{xdrv}:\''';'shell batfl'
6!:2 'shell ''xcopy ',BASEDSK,'\Temp\{machid}{todayDt} {xdrv}:\Temp\{machid}{todayDt} /S /C /H /I /R /Q /Y'''
80 (] {."1~ [ <. [: {: [: $ ]) ":,.rr=. ".&.> 0 2 3 _4 _3 { <;._2 usuReminder [ smoutput qts'' NB. Max 80 cols display
...
NB.* buildBatFl: build .BAT file to create target dirs and copy files to them.
   ...
   ix=. figureWhichFiles szlim;<dtdord
   NB. 0{DIRNMS, up to ':', gives disk from which to copy.
   cmds=. (<'//';'/') rplc~&.>(({.~>:@:i.&':')0{DIRNMS),makeCopyCmds targ;<ix
