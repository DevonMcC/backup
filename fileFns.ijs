...
NB.* freeSpaceOn: bytes of free space on disk (letter) y.
...
NB.EG 6!:2 '(3 : ''shell ''''rmxs && del "#*#" && del ".#*"''''[1!:44 y'')generalWalkTree ''C:\amisc'''
...
NB.* dirSize: total size of all files and all subdirectories under diry y
dirSize=: 3 : 0
...
NB.   y=. PATHSEP_j_ (],[#~[~:[:{:]) y
   ...
   if. x do. +/szs else. ,szs end.
NB.EG dirSize 'C:\Program Files'
)
...
usualUse=: 0 : 0
   6!:2 'rr=. a:-.~'''' lookUnder ''c:/ClarifiWork/2019.07/Vendors/XFv5/SQL Server'''
   6!:2 '(3 : ''shell ''''rmxs && del "#*#" && del ".#*"''''[1!:44 y'')generalWalkTree ''C:'''
